...
1.	ВВЕДЕНИЕ
	...
	Задачи
2.  Основная часть
2.1Анализ предметной области. Постановка задачи
2.2Инфологическая (концептуальная) модель базы данных
2.3Логическая структура БД
2.4Физическая структура базы данных
2.5Реализация проекта в среде конкретной СУБД	
...
2.	Определение атрибутов объектов. Каждая из выделенных сущностей имеют свои уникальные атрибуты, которые описывают их свойства.
  ...
  - **Время:**
    ...
    -	mounth (Месяц проведения операции/учета).
    -	year (Год проведения операции/учета).
